...
function Login({ contas, onOk, onReg }) {
  const { t } = useT();
  ...
  function go() {
    ...
    const c = contas.find(x => x.email === e.toLowerCase() && x.senha === s);
    if (!c) { setEr(t("l_invalid")); return; }
    onOk(c);
  }
  return (
    <div style={{ background: "var(--bg0)", borderRadius: 12, overflow: "hidden" }}>
      ...
      <div style={{ padding: 20, maxWidth: 360, margin: "0 auto" }}>
        ...
        <Card style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: "#f5c518", marginBottom: 4 }}>{t("l_demo")}</div>
          {DEMOS.map(c => (
            <div key={c.email} onClick={() => { setE(c.email); setS(c.senha); }} style={{ cursor: "pointer", fontSize: 11, display: "flex", justifyContent: "space-between", padding: "2px 0", color: "#58a6ff" }}>
              <span>{c.email}</span>
              <span style={{ color: PLANS.find(p => p.id === c.plano).cor }}>{PLAN_NAMES[c.plano]}</span>
            </div>
          ))}
        </Card>
        ...
        <Btn primary onClick={go} style={{ width: "100%", marginBottom: 10 }}>{t("w_login")}</Btn>
...
function Reg({ onOk, onLog }) {
  ...
  function go() {
    ...
    onOk({ nome: n, email: e.toLowerCase(), senha: s, plano: "gratuito" });
  }
  return (
    <div style={{ background: "var(--bg0)", borderRadius: 12, overflow: "hidden" }}>
      <div style={{ background: "var(--bg1)", borderBottom: "1px solid var(--ovB)", padding: "10px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      ...
      <div style={{ padding: 20, maxWidth: 360, margin: "0 auto" }}>
        ...
        {er && <div style={{ fontSize: 11, color: "#f85149", marginBottom: 8 }}>{er}</div>}
        <Btn primary onClick={go} style={{ width: "100%", marginBottom: 10 }}>{t("r_create")}</Btn>
...
function AppInner() {
  const { t } = useT();
  const [contas, setContas] = useState(DEMOS);
  ...
  function login(c) { setAuth(c); setTela("app"); setSec("home"); }
  function reg(c) { setContas(p => [...p, c]); setAuth(c); setTela("app"); setSec("home"); }
  function logout() { setAuth(null); setTela("welcome"); }
  function upgrade(p) { setAuth(a => ({ ...a, plano: p })); setContas(prev => prev.map(c => (auth && c.email === auth.email) ? { ...c, plano: p } : c)); }

  if (tela === "welcome") return <Welcome onL={() => setTela("login")} onR={() => setTela("reg")} onGuest={() => setTela("guest")} />;
  if (tela === "login") return <Login contas={contas} onOk={login} onReg={() => setTela("reg")} />;
  if (tela === "reg") return <Reg onOk={reg} onLog={() => setTela("login")} />;
